  ...
  <div class="h-full flex flex-col bg-zinc-100">
    ...
    <div class="projects-window h-full p-4 md:p-6 flex-1 overflow-y-auto">
      ...
      <div v-if="loading" class="flex items-center justify-center h-full">
        <div
          class="animate-spin rounded-full h-4 w-4 border-t-2 border-b-2 border-zinc-300 mr-2"
        ></div>
      ...
      <div v-else-if="error" class="flex flex-col items-center justify-center h-full text-zinc-600">
        ...
        <p>{{ error }}</p>
        <button
          class="bg-zinc-200 hover:bg-zinc-300 text-zinc-800 pb-1 px-4 rounded mt-4"
          @click="fetchRepos"
        >
          Try Again
        </button>
              ...
              <div class="flex justify-between items-start mb-4">
                ...
                <div class="flex gap-1 -mr-1">
                  <a
                    ...
                    class="text-zinc-500 hover:text-zinc-700 transition-colors p-1 -mr-1"
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    class="text-zinc-500 hover:text-zinc-700 transition-colors p-1"
                    title="Live Demo"
                  >
                    <svg
                      v-once
...
const fetchRepos = async () => {
  ...
  try {
    ...
    const response = await fetch(
      "https://api.github.com/users/chenxing-dev/repos?sort=updated&per_page=20",
    );
    if (!response.ok) {
